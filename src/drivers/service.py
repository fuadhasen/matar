...
class DriverService:
    ...
    async def get_a_driver(self, driver_id: UUID, session: AsyncSession):
    ...
    async def update_driver(self, driver_id: UUID, driver_data: DriverCreateModel, session: AsyncSession):
        driver = self.get_a_driver(driver_id)
        if not driver:
            raise HTTPException(
    ...
    async def delete_driver(self, driver_id: UUID, session: AsyncSession):
        driver = self.get_a_driver(driver_id)
        if not driver:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
